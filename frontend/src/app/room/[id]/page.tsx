    ...
    const handleAnswer = async (data: IAnswer) => {
        ...
        if (data.description.type === 'offer') {
            ...
            socket?.emit('sdp', {
                to: data.sender,
                sender: socket?.id,
                descrition: peerConnection.localDescription
            });
    ...
    const createPeerConnection = async (socketId: string, createOffer: boolean)=> {
        ...
        if (createOffer) {
            ...
            socket?.emit('sdp', {
                to: socketId,
                sender: socket?.id,
                description: peerConnection.setLocalDescription,
            });
        }
                ...
                <div className="md:w-[85%] w-full m-3">
                    <div className="grid md:grid-cols-2 grid-cols-1 gap-8">
                        <div className='bg-gray-950 w-full rounded-md h-full p-2 relative'>
                            <video className="h-full w-full mirror-mode" autoPlay ref={localStream}></video>
                            <span className="absolute bottom-3">Misha</span>
                        </div>
                        {remoteStreams.map((stream, index)=> {
                            return (
                                ...
                                <span className="absolute bottom-3">Jean</span>
                                </div>
                            );
                        })}
                    </div>
